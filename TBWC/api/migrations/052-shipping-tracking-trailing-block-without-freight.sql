-- Fixes order.shipping_tracking (migration 051): some invoices fold freight
-- into a lump-sum "LOT" line instead of a dedicated FREIGHT item, but still
-- end the invoice with the same no-Item shipping note (e.g. invoice 10377:
-- last real item is "LOT", then a blank line, then "Tracking:\n1ZK6B2...").
-- Migration 051 only picked up a trailing note anchored to a FREIGHT item, so
-- these were silently dropped. This re-runs the refresh with the invoice's
-- trailing block also counted whenever it actually has a continuation line
-- (more than one row) -- see orderInvoiceStatus.ts for the query this
-- mirrors. A last item with no trailing no-Item line is just that item's own
-- description and is correctly left out.
    ...
    SELECT i.ref_number, i.is_paid,
           ...
           (WITH lo AS (
              ...
              SELECT lo.block_id,
                     ...
                     count(*) AS row_count,
                     string_agg(lo.desc, '; ' ORDER BY lo.ord) FILTER (WHERE lo.desc IS NOT NULL AND lo.desc <> '') AS block_desc
                FROM lo
            ...
            SELECT string_agg(blk.block_desc, '; ' ORDER BY blk.block_id)
             ...
             WHERE blk.is_freight
                OR (blk.row_count > 1 AND blk.block_id = (SELECT max(block_id) FROM blk))
